...
usage: nc-execute-xr-ping-act-26-ydk.py [-h] [-v] device
...
def process_ping(ping):
    ...
    # format string for reply row
    ping_reply_row = '{result}\n'
    # format string for reply trailer
    ping_reply_trailer = ('Success rate is {success_rate} percent '
                          '({hits}/{total}), '
    ...
    for reply in ping_response.ipv4[0].replies.reply:
        ping_reply += ping_reply_row.format(result=reply.result)
