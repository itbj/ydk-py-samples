...
def validate_peer_interface(kafka_consumer, node, name,
    ...
    for kafka_msg in kafka_consumer:
        ...
        if (msg["Telemetry"]["node_id_str"] == node and
                ...
                and "Rows" in msg):
            for row in msg["Rows"]:
                if ("name" in row["Keys"] and
                        row["Keys"]["name"] == name and
                        "state" in row["Content"] and
                        "admin-status" in row["Content"]["state"] and
                        row["Content"]["state"]["admin-status"] == status and
                        "oper-status" in row["Content"]["state"] and
                        row["Content"]["state"]["oper-status"] == status):
                    return True
